...
using Brazilla.Services.UserServices.Helpers;
    ...
    public class CurrentUser : ICurrentUser
    {
        ...
        public string? Login
        {
            get
            {
                try
                {
                    return _httpContextAccessor?.HttpContext?.User.Claims?.FirstOrDefault(user => user.Type == Claims.LOGIN)?.Value;
                }
                catch(NullReferenceException ex)
                {
                    return null;
        ...
        public UserTypes? Role
        {
            get
            {
                try
                {
                    return UserTypeHelper.GetUserTypeAsEnum(_httpContextAccessor?.HttpContext?.User?.Claims?.FirstOrDefault(user => user.Type == Claims.ROLE)?.Value);
                }
                catch(NullReferenceException ex)
                {
                    return null;
